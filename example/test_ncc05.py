...
class TestNCC05(unittest.TestCase):
    def test_publish_and_resolve(self):
        ...
        async def run_test():
            ...
            await publish_run(
                ...
                relay=test_relay, d_tag=unique_d
            )
            await asyncio.sleep(2)
            payload = await resolve_run(
                provided_keys=keys, target_pk=pk,
                bootstrap_relay=test_relay, identifier=unique_d
            )
    ...
    def test_gossip_discovery(self):
        ...
        async def run_test():
            ...
            # 1. Publish Relay List to Relay A
            print(f"[Gossip Test] Publishing Relay List to {relay_a}...")
            await publish_run(
                provided_keys=keys, relay=relay_a, relay_list=relay_b
            )
            await asyncio.sleep(2)

            # 2. Publish actual record to Relay B
            print(f"[Gossip Test] Publishing NCC-05 record to {relay_b}...")
            await publish_run(
                provided_keys=keys, manual_ip=test_ip,
                relay=relay_b, d_tag=unique_d
            )
            ...
            print(f"[Gossip Test] Resolving via {relay_a} with Gossip...")
            payload = await resolve_run(
                provided_keys=keys, target_pk=pk,
                bootstrap_relay=relay_a, gossip=True, identifier=unique_d
            )

            self.assertIsNotNone(payload, "Gossip failed to find payload")
            self.assertTrue(any(test_ip in ep['uri']
                            for ep in payload['endpoints']))
            print("[Gossip Test] Success!")
