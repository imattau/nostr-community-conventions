...
- `relayMode`: Controls whether the sidecar advertises endpoints. Set to `"public"` (default) to publish NCC-05 locators, or `"private"` to publish only the NCC-02 service record (the locator/encrypted locator steps are skipped while attestations/revokes still exist).
...
- `ncc06-sidecar/config.json` captures the service identity (`serviceSk`, `servicePk`, `serviceNpub`), NCC-02/NCC-05 metadata, and the new `k` block. `k.mode` lets you choose `tls_spki` (default when `wss://` endpoints exist), `static`, or `generate`, plus the supporting `certPath`, `value`, or `persistPath` values. The harness already seeds the TLS fingerprint for local dev, but you can override via `NCC06_K_MODE` / `NCC06_NCC02_KEY_SOURCE`. Set `relayMode` (or `NCC06_RELAY_MODE`) to `"public"` or `"private"` to control whether NCC-05 locators are published; private relays still emit NCC-02/attestation/revocation events but do not advertise endpoints.
