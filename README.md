...
Service Records, published by a service owner
Certificate Attestations, optionally published by third parties
Revocations, published by certifiers
...
Query multiple relays for the current Service Record for (pubkey, service-id)
Verify signature and expiry
Fetch Certificate Attestations referencing that record
Filter attestations by trusted certifier pubkeys and validity
Apply local policy requirements
Connect to the endpoint
Verify the endpoint key matches fingerprint k
Fail closed on mismatch unless explicitly overridden by the user
Trust Model
